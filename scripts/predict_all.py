...
PREDICT_FOLDER = os.path.join(os.path.abspath(
    os.path.dirname(os.path.dirname(__file__))), 'src/static/predict')
...
def get_predict_file(PREDICT_FOLDER):
    predict_path = []
    for i in os.listdir(PREDICT_FOLDER):
        predict_path.append(os.path.join(PREDICT_FOLDER, i))
    return predict_path
...
pre_path = get_predict_file(PREDICT_FOLDER)

# recognizer.read(PREDICT_FOLDER+'/19520229.yml')
